...
public class resetUserView {
	JFrame userframe = new JFrame("重置用户口令界面");
	JPanel userPanel;
	JLabel userLabel;
	JTextField userText;
	...
	private void init()
	{
	   ...
	   // userframe.add(deleteUser);
        userframe.setVisible(true);
        resetUserButton = new JButton("确定重置");
	    ...
	    resetUserButton.addActionListener(new ActionListener(){
	        ...
	        public void actionPerformed(ActionEvent e) {
	        	...
	        	int result = JOptionPane.showConfirmDialog(null, "确定重置用户:"+ deletename +" 的口令？", "是否重置？", JOptionPane.YES_NO_OPTION);
	        	if(result == 0)
	        	{
	...
	private  String[] queryUser()  //查询数据库中的用户
    {  
	...
	private boolean resetUser(String userName)  //重置指定用户的口令 将其口令重置为123456
    {   
        ...
        try{  
            ...
            JOptionPane.showMessageDialog(null, "成功重置用户口令"+ userName +"!");
            pstmt.close();          
            conn.close();   
            return true;              
        }  
